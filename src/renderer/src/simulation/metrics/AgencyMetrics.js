 ...
 */
export function computeEntropy(tasks) {
    if (tasks.length === 0)
        return 0;
 ...
 */
export function computeEnergyEfficiency(entropyPrev, entropyCurrent, energySpent) {
 ...
 */
export function computeBehavioralVariance(policyPrev, policyCurrent) {
    if (!policyPrev || !policyCurrent || policyPrev.length !== policyCurrent.length)
        return 0;
    let sumSq = 0;
 ...
 */
export function computeFeedbackGain(policyChangeMagnitude, // BV
scoreDelta) {
 ...
 */
export function computeEmergentIntention(EE, AFG, BV, TaskProgress, // Normalized 0-1
